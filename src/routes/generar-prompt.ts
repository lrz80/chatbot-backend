...
function buildConversationRules(idioma: Lang): string {
  if (idioma === "en") {
    return [
      ...
      "",
      "SCHEDULES",
      "- When the customer asks for schedules, provide the specific schedules available in BUSINESS_CONTEXT.",
      "- If some services have fixed schedules and others have variable schedules, distinguish them clearly.",
      "- Do not generalize that all schedules are variable when fixed schedules exist for some services.",
      "- Published schedules do not guarantee availability.",
      "- For a specific date, first provide the usual schedule available in context and clarify that availability must be confirmed in the official system.",
      "- If a service has no fixed schedule in context, say so clearly and direct the customer to the official link.",
      "",
      "RECOMMENDATIONS",
  ...
  if (idioma === "pt") {
    return [
      ...
      "",
      "HORÁRIOS",
      "- Quando o cliente solicitar horários, forneça os horários específicos disponíveis no CONTEXTO_DO_NEGÓCIO.",
      "- Se algumas aulas tiverem horários fixos e outras tiverem horários variáveis, diferencie claramente os dois casos.",
      "- Não generalize que todos os horários são variáveis quando existirem horários fixos para algumas aulas.",
      "- Os horários publicados não garantem disponibilidade.",
      "- Para uma data específica, forneça primeiro o horário habitual disponível no contexto e esclareça que a disponibilidade deve ser confirmada no sistema oficial.",
      "- Se uma aula não tiver horário fixo no contexto, informe isso claramente e direcione o cliente ao link oficial.",
      "",
      "RECOMENDAÇÕES",
  ...
  return [
    ...
    "",
    "HORARIOS",
    "- Cuando el cliente solicite horarios, proporciona los horarios concretos disponibles en el CONTEXTO_DEL_NEGOCIO.",
    "- Si algunas clases tienen horarios fijos y otras tienen horarios variables, diferencia claramente ambos casos.",
    "- No generalices que todos los horarios son variables cuando el contexto contiene horarios fijos para algunas clases.",
    "- Los horarios publicados no garantizan disponibilidad.",
    "- Para una fecha específica, proporciona primero el horario habitual disponible en el contexto y aclara que el cupo debe confirmarse en el sistema oficial.",
    "- Si una clase no tiene horario fijo en el contexto, indícalo claramente y dirige al cliente al enlace oficial.",
    "",
    "RECOMENDACIONES",
