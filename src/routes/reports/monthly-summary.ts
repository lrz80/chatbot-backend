...
function parseMonth(value: unknown): { start: string; end: string; label: string } | null {
...
router.get("/monthly-summary", authenticateUser, async (req: Request, res: Response) => {
  try {
    ...
    if (!month) {
      return res.status(400).json({
        error: "Parámetro month inválido. Usa formato YYYY-MM. Ejemplo: 2026-07",
      });
    }
    ...
    const [
      ...
      pool.query(
        ...
        SELECT
          ...
          COUNT(*) FILTER (
            WHERE LOWER(COALESCE(status, '')) NOT IN ('cancelled', 'canceled', 'failed', 'error')
    ...
    for (const row of channelRows.rows) {
      conversationsByChannel[row.channel] = toNumber(row.total);
    }
    ...
    const bookingsStarted = toNumber(bookingsStartedSummary.rows[0]?.bookings_started);
    const appointmentsCreated = toNumber(appointmentsSummary.rows[0]?.total_appointments);
    const bookingsConfirmed = toNumber(appointmentsSummary.rows[0]?.successful_appointments);
