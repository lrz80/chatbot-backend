...
"¿Sobre qué te gustaría saber más? ¿Servicios, precios, u otra cosa?"`;
  ...
  // ===============================
  // ✅ DEMO FLOW STATEFUL (sin helpers externos)
  // Usa clientes.estado para evitar loop del demo
  // Estados:
  // - demo_canal
  // - demo_tipo_whatsapp | demo_tipo_instagram | demo_tipo_facebook
  // ===============================
  {
    const tenantId = tenant.id;
    const canalEnvio = canal;      // 'whatsapp'
    const senderId = contactoNorm; // contacto normalizado

    // Lee estado actual del cliente (si existe)
    let estadoDemo: string | null = null;
    try {
      const { rows } = await pool.query(
        `SELECT estado
          FROM clientes
      ...
      estadoDemo = rows[0]?.estado ? String(rows[0].estado) : null;
    } catch {}

    const msgLower = (userInput || "").trim().toLowerCase();

    const parseCanalElegido = (t: string) => {
      if (t.includes("whats")) return "whatsapp";
      if (t.includes("insta")) return "instagram";
      if (t.includes("face") || t.includes("fb")) return "facebook";
      return null;
    };

    const isDemoTrigger =
      /\b(demo|demostraci[oó]n|probar|test|trial)\b/i.test(msgLower) &&
      /\b(aamy|amy)\b/i.test(msgLower);

    // 1) Si activa demo por primera vez → set demo_canal y preguntar canal
    if (isDemoTrigger && !estadoDemo) {
      await pool.query(
        `INSERT INTO clientes (tenant_id, canal, contacto, estado, updated_at)
        VALUES ($1, $2, $3, 'demo_canal', now())
        ON CONFLICT (tenant_id, canal, contacto)
        DO UPDATE SET estado='demo_canal', updated_at=now()`,
        [tenantId, canalEnvio, senderId]
      );

      const reply =
        idiomaDestino === "en"
          ? "Hi! Welcome to the Aamy demo. Which channel do you want to automate: WhatsApp, Instagram, or Facebook?"
          : "¡Hola! Bienvenido al demo de Aamy. ¿Qué canal quieres automatizar: WhatsApp, Instagram o Facebook?";

      const ok = await safeEnviarWhatsApp(tenantId, canalEnvio, messageId, fromNumber, reply);
      if (ok) {
        await saveAssistantMessageAndEmit({
          tenantId,
          canal: canalEnvio,
    ...
    // 2) Si estaba esperando canal → si elige, avanzamos a pedir tipo de negocio
    if (estadoDemo === "demo_canal") {
      const elegido = parseCanalElegido(msgLower);
      ...
      await pool.query(
        `UPDATE clientes
            SET estado = $4,
                updated_at = now()
          WHERE tenant_id = $1 AND canal = $2 AND contacto = $3`,
        [tenantId, canalEnvio, senderId, `demo_tipo_${elegido}`]
      );

      const reply =
        idiomaDestino === "en"
          ? `Perfect. For ${elegido}, what type of business is it? (e.g., bakery, barber shop, gym, real estate)`
          : `Perfecto. Para ${elegido}, ¿qué tipo de negocio es? (ej: panadería, barbería, gimnasio, realtor)`;
    ...
    // 3) Si ya eligió canal y ahora esperamos tipo de negocio → respondemos demo y limpiamos estado
    if (estadoDemo && estadoDemo.startsWith("demo_tipo_")) {
      const elegido = estadoDemo.replace("demo_tipo_", "").trim(); // whatsapp|instagram|facebook
      const tipoNegocio = (userInput || "").trim();

      // Limpia estado demo (para que no se repita)
      await pool.query(
        `UPDATE clientes
            SET estado = NULL,
                updated_at = now()
      ...
      const reply =
        idiomaDestino === "en"
          ? `Great. For a ${tipoNegocio}, Aamy can automate ${elegido} by answering FAQs, capturing leads (name/phone), and following up automatically when the customer stops replying. Do you want the goal to be “sell”, “book appointments”, or “support” (FAQ)?`
          : `Perfecto. Para un negocio tipo "${tipoNegocio}", Aamy puede automatizar ${elegido} respondiendo FAQs, capturando leads (nombre/teléfono) y dando seguimiento automático si el cliente deja de responder. ¿Tu objetivo es “vender”, “agendar citas” o “soporte” (FAQ)?`;
