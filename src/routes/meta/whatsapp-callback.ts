...
/**
 * Este endpoint es el "Redirect / Callback URL" configurado en Meta
...
router.get("/whatsapp/callback", async (req: Request, res: Response) => {
  try {
    ...
    const phoneNumberId =
      q.phone_number_id || q.wa_phone_number_id || null;
    ...
    // Ajusta los nombres de columnas si son distintos en tu esquema
    await pool.query(
      `
        UPDATE tenants
        SET
