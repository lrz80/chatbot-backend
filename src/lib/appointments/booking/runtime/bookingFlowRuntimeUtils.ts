...
export function buildAnswersBySlot(params: {
   ...
   * Tool metadata must never become slot answers.
   */
...
export function resolveCurrentStepIndex(params: {
  ...
  if (explicitStepKey) {
    const explicitIndex = getStepIndexByKey(
      steps,
      explicitStepKey
    );
  ...
  if (missingRequired.length > 0) {
    const index = getStepIndexByKey(
      steps,
      clean(missingRequired[0].step_key)
    );
...
export function buildCanonicalCallState<
  T extends BookingRuntimeState
>(params: {
  state: T;
  answersBySlot: Record<string, string>;
  ...
  return {
    ...
    bookingStepIndex:
      typeof bookingStepIndex === "number"
        ? bookingStepIndex
        : undefined,
