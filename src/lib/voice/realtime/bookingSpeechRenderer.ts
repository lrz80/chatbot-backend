...
export function buildBookingSpeechResponse(
  ...
  return {
    ...
    instructions: [
      ...
      "You may lightly rephrase the wording to sound human.",
      "You must preserve all required details from the original booking prompt.",
      "You must preserve the final question or confirmation request if one exists.",
      "Do not shorten the prompt by removing meaningful content.",
      "",
        ...
        content: [
          {
            type: "input_text",
            text: [
            "Convert the booking prompt into a natural spoken version.",
            "Preserve every required detail, clause, condition, and final question from the prompt.",
            "Do not omit any sentence or instruction from the original prompt.",
            "Do not add new facts.",
            "Return only the spoken response.",
            "",
            `Original booking prompt: ${prompt}`,
            ].join("\n"),
          },
        ],
      },
