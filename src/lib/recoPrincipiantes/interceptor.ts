...
import { loadTiposClases } from './tiposClases';
...
// helper: detecta si el tenantId “parece” uuid
...
export async function runBeginnerRecoInterceptor(args: InterceptorArgs): Promise<boolean> {
  ...
  // 0) Normaliza pregunta e intención final
  const preguntaNormalizada = userInput
    .toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim();
  const intencionFinal = 'duda__recomendacion_principiantes';

  // 1) Registrar FAQ sugerida (evita duplicados exactos no procesados)
  //    Hacemos dos variantes de SQL según sea uuid o no.
  ...
  let tipos: any[] = [];
  try {
    tipos = await loadTiposClases(tenantId);
  } catch (e) {
  ...
  console.log('🎯 BeginnerReco: tipos_clases cargados', {
    tenantId,
    canal,
    fuente: (tipos as any).__source || 'desconocida',
    total: tipos?.length ?? 0,
    preview: Array.isArray(tipos) ? tipos.slice(0, 3) : tipos
  });
  ...
  // 3.1) Fallback si no hay tipos o el mensaje vino vacío
  if (!msg || !msg.trim()) {
  ...
  // 4) Añadir (si existe) FAQ oficial 'interes_clases'
  try {
    ...
    if (rows[0]?.respuesta) {
      ...
      // Evita duplicar el mismo bloque si el builder ya lo incluyó
  ...
  // 5) Cinturón de seguridad: garantizar idiomaDestino
  try {
